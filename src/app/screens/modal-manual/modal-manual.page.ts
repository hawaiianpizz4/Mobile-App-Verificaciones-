...
export class ModalManualPage implements OnInit {
  gestor = JSON.parse(localStorage.getItem('user')).toUpperCase();
  ...
  submitForm(e) {
    ...
    if (
      ...
      if (this.status) {
        ...
        var data = `{"cedula":"${cedula.value}","operacion":"${numCredito.value}","gestion":"${gestion.value}","cobranza":"${cobranza.value}","observacion":"${observacion.value}","contacto":"${contacto.value}","plazoNuevo":${plazoInsert},"valorRenegocio":${valorInsert},"latitud":${lat.value},"longitud":${long.value},"gestor":"${this.gestor}"}`;
